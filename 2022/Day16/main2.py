...
def solve1(node1, node2):
    global time1, preassure, max_preassure, visited
    if time1 <= 0:
        return
    ...
    for neighbor in non_zero:
        if visited[neighbor]:
            continue
        time1 -= (dist[node1][neighbor]+1)
        preassure += time1*G_flow[neighbor]
        visited[neighbor] = True
        solve2(neighbor, node2)
        visited[neighbor] = False
        preassure -= time1*G_flow[neighbor]
        time1 += (dist[node1][neighbor]+1)


def solve2(node1, node2):
    global time2, preassure, max_preassure, visited
    if time2 <= 0:
        return
    max_preassure = max(max_preassure, preassure)
    for neighbor in non_zero:
        if visited[neighbor]:
            continue
        time2 -= (dist[node2][neighbor]+1)
        preassure += time2*G_flow[neighbor]
        visited[neighbor] = True
        solve1(node1, neighbor)
        visited[neighbor] = False
        preassure -= time2*G_flow[neighbor]
        time2 += (dist[node2][neighbor]+1)
